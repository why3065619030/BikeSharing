...
public class MiuiWeatherView extends View {
    ...
    private List<Pair<Integer, String>> weatherDatas = new ArrayList<>();  //对元数据中分组后的集合
    private List<Float> dashDatas = new ArrayList<>(); //之间虚线的x坐标集合
    private List<PointF> points = new ArrayList<>(); //折线拐点的集合
    private Float maxTemperature;//元数据中的最高和最低温度
    ...
    private void calculatePontGap() {
        Float lastMaxTem = -Float.MAX_VALUE;
        Float lastMinTem = Float.MAX_VALUE;
    ...
    private void drawLinesAndPoints(Canvas canvas) {
        ...
        float centerY;
        for (int i = 0; i < data.size(); i++) {
            Float tem = data.get(i).temperature;
